...
class RudinOwnVarReward:
    def __init__(self, env,
                 curriculum_steps=15e6,
                 tracking_xy_velocity_command_coeff=4.0, tracking_yaw_velocity_command_coeff=2.0,
                 ...
                 collision_coeff=1e0, base_height_coeff=3e1, nominal_trunk_z=0.161, air_time_coeff=1e-1, air_time_max=0.5, symmetry_air_coeff=0.5):
        self.env = env
        ...
        self.time_since_last_touchdown_f0 = 0
        self.time_since_last_touchdown_f1 = 0
        self.time_since_last_touchdown_f2 = 0
        self.time_since_last_touchdown_f3 = 0
        self.time_since_last_touchdown_f4 = 0
        self.time_since_last_touchdown_f5 = 0
    ...
    def step(self, action):
        self.time_since_last_touchdown_f0 = 0 if self.env.check_collision("floor", "foot_0") else self.time_since_last_touchdown_f0 + self.env.dt
        self.time_since_last_touchdown_f1 = 0 if self.env.check_collision("floor", "foot_1") else self.time_since_last_touchdown_f1 + self.env.dt
        self.time_since_last_touchdown_f2 = 0 if self.env.check_collision("floor", "foot_2") else self.time_since_last_touchdown_f2 + self.env.dt
        self.time_since_last_touchdown_f3 = 0 if self.env.check_collision("floor", "foot_3") else self.time_since_last_touchdown_f3 + self.env.dt
        self.time_since_last_touchdown_f4 = 0 if self.env.check_collision("floor", "foot_4") else self.time_since_last_touchdown_f4 + self.env.dt
        self.time_since_last_touchdown_f5 = 0 if self.env.check_collision("floor", "foot_5") else self.time_since_last_touchdown_f5 + self.env.dt
    ...
    def reward_and_info(self, info, done):
        ...
        collisions = self.env.check_any_collision_for_all([
            "hip_0", "hip_1", "hip_2", "hip_3", "hip_4", "hip_5",
            "knee_0", "knee_1", "knee_2", "knee_3", "knee_4", "knee_5",
        ])
        trunk_collision = 1 if self.env.check_any_collision(["trunk_1", "trunk_2", "trunk_3", "trunk_4", "trunk_5", "trunk_6", "trunk_7"]) else 0
        ...
        # Air time reward
        air_time_reward = 0.0
        foot_0_on_ground = self.env.check_collision("floor", "foot_0")
        foot_1_on_ground = self.env.check_collision("floor", "foot_1")
        foot_2_on_ground = self.env.check_collision("floor", "foot_2")
        foot_3_on_ground = self.env.check_collision("floor", "foot_3")
        foot_4_on_ground = self.env.check_collision("floor", "foot_4")
        foot_5_on_ground = self.env.check_collision("floor", "foot_5")
        if foot_0_on_ground:
            air_time_reward += self.time_since_last_touchdown_f0 - self.air_time_max
        if foot_1_on_ground:
            air_time_reward += self.time_since_last_touchdown_f1 - self.air_time_max
        if foot_2_on_ground:
            air_time_reward += self.time_since_last_touchdown_f2 - self.air_time_max
        if foot_3_on_ground:
            air_time_reward += self.time_since_last_touchdown_f3 - self.air_time_max
        if foot_4_on_ground:
            air_time_reward += self.time_since_last_touchdown_f4 - self.air_time_max
        if foot_5_on_ground:
            air_time_reward += self.time_since_last_touchdown_f5 - self.air_time_max
        air_time_reward = curriculum_coeff * self.air_time_coeff * air_time_reward

        # Symmetry reward
        symmetry_air_violations = 0.0
        if not foot_0_on_ground and not foot_1_on_ground:
            symmetry_air_violations += 1
        if not foot_2_on_ground and not foot_3_on_ground:
            symmetry_air_violations += 1
        if not foot_4_on_ground and not foot_5_on_ground:
            symmetry_air_violations += 1
