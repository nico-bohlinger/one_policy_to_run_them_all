...
class RudinOwnVarReward:
    def __init__(self, env,
                 ...
                 z_velocity_coeff=2e0, pitch_roll_vel_coeff=5e-2, pitch_roll_pos_coeff=2e-1, joint_nominal_diff_coeff=2e-1,
                 joint_position_limit_coeff=1e1, soft_joint_position_limit=0.9,
                 joint_velocity_coeff=0.0, joint_acceleration_coeff=2.5e-7, joint_torque_coeff=5e-5, action_rate_coeff=1e-2,
                 collision_coeff=1e0, base_height_coeff=3e1, nominal_trunk_z=0.72, air_time_coeff=1e-1, air_time_max=0.5, symmetry_air_coeff=0.5):
        self.env = env
    ...
    def reward_and_info(self, info, done):
        ...
        joint_nominal_diff_norm = np.sum(np.square(self.env.data.qpos[self.env.joint_mask_qpos][12:] - self.env.domain_randomization_seen_robot_function.seen_joint_nominal_position[12:]))
