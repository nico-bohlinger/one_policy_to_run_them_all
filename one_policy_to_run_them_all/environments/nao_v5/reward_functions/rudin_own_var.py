...
class RudinOwnVarReward:
    def __init__(self, env,
                 ...
                 z_velocity_coeff=2e0, pitch_roll_vel_coeff=1e-1, pitch_roll_pos_coeff=2e-1, joint_nominal_diff_coeff=1.5e-1,
                 joint_position_limit_coeff=1e1, soft_joint_position_limit=0.9,
                 joint_velocity_coeff=0.0, joint_acceleration_coeff=1.2e-6, joint_torque_coeff=4e-4, action_rate_coeff=6e-3,
                 collision_coeff=1e0, base_height_coeff=3e1, nominal_trunk_z=0.32, air_time_coeff=1e-1, air_time_max=0.5, symmetry_air_coeff=0.5):
        self.env = env
    ...
    def step(self, action):
        self.time_since_last_touchdown_left = 0 if self.env.check_collision("floor", "left_foot") else self.time_since_last_touchdown_left + self.env.dt
        self.time_since_last_touchdown_right = 0 if self.env.check_collision("floor", "right_foot") else self.time_since_last_touchdown_right + self.env.dt
    ...
    def reward_and_info(self, info, done):
        ...
        joint_nominal_diff_norm = np.sum(np.square(self.env.data.qpos[self.env.joint_mask_qpos][[0, 1, 14, 15, 16, 17, 18, 19, 20, 21]] - self.env.domain_randomization_seen_robot_function.seen_joint_nominal_position[[0, 1, 14, 15, 16, 17, 18, 19, 20, 21]]))
        ...
        # Air time reward
        foot_right_on_ground = self.env.check_collision("floor", "right_foot")
        foot_left_on_ground = self.env.check_collision("floor", "left_foot")
        air_time_reward = 0.0
