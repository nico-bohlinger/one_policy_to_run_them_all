...
class RudinOwnVarReward:
    def __init__(self, env,
                 curriculum_steps=200e6,
                 tracking_xy_velocity_command_coeff=6.0, tracking_yaw_velocity_command_coeff=3.0,
                 xy_tracking_temperature=0.25, yaw_tracking_temperature=0.25,
                 z_velocity_coeff=2e0, pitch_roll_vel_coeff=5e-2, pitch_roll_pos_coeff=2e-1, joint_nominal_diff_coeff=5e-2,
                 ...
                 collision_coeff=1e0, base_height_coeff=3e1, nominal_trunk_z=0.87, air_time_coeff=1e-1, air_time_max=0.5, symmetry_air_coeff=0.5):
        self.env = env
    ...
    def reward_and_info(self, info, done):
        ...
        joint_nominal_diff_norm = np.sum(np.square(self.env.data.qpos[7:22] - self.env.domain_randomization_seen_robot_function.seen_joint_nominal_position[:15]))
