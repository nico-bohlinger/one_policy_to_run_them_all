...
class RudinOwnVarReward:
    def __init__(self, env,
                 ...
                 collision_coeff=1e0, base_height_coeff=3e1, nominal_trunk_z=0.55, air_time_coeff=1e-1, air_time_max=0.5, symmetry_air_coeff=0.5):
        self.env = env
        ...
        self.time_since_last_touchdown_lh = 0
        self.time_since_last_touchdown_rh = 0
    ...
    def step(self, action):
        ...
        self.time_since_last_touchdown_lh = 0 if self.env.check_collision("floor", "LH_foot") else self.time_since_last_touchdown_lh + self.env.dt
        self.time_since_last_touchdown_rh = 0 if self.env.check_collision("floor", "RH_foot") else self.time_since_last_touchdown_rh + self.env.dt
    ...
    def reward_and_info(self, info, done):
        ...
        collisions = self.env.check_any_collision_for_all([
            "LF_kfe_act", "RF_kfe_act", "LH_kfe_act", "RH_kfe_act",
            "LF_calf", "RF_calf", "LH_calf", "RH_calf",
        ])
        trunk_collision = 1 if self.env.check_any_collision(["trunk_1", "trunk_2", "trunk_3", "trunk_4", "trunk_5", "trunk_6", "trunk_7", "trunk_8"]) else 0
        ...
        foot_lh_on_ground = self.env.check_collision("floor", "LH_foot")
        foot_rh_on_ground = self.env.check_collision("floor", "RH_foot")
        ...
        if foot_lh_on_ground:
            air_time_reward += self.time_since_last_touchdown_lh - self.air_time_max
        if foot_rh_on_ground:
            air_time_reward += self.time_since_last_touchdown_rh - self.air_time_max
        air_time_reward = curriculum_coeff * self.air_time_coeff * air_time_reward

        # Symmetry reward
        symmetry_air_violations = 0.0
        if not foot_lf_on_ground and not foot_rf_on_ground:
            symmetry_air_violations += 1
        if not foot_lh_on_ground and not foot_rh_on_ground:
            symmetry_air_violations += 1
